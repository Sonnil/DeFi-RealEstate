function $(e){return document.getElementById(e)}function storageGet(e,t){try{const n=localStorage.getItem(e);return n?JSON.parse(n):t}catch{return t}}function storageSet(e,t){try{localStorage.setItem(e,JSON.stringify(t))}catch{}}function prefersReducedMotion(){return window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function injectHeaderFooter(e){
// Ensure hamburger-only nav on touch devices regardless of orientation
try{if(!document.getElementById("dre-mobile-nav-style")){const e=document.createElement("style");e.id="dre-mobile-nav-style",e.textContent="@media (hover:none) and (pointer:coarse){header .nav nav{display:none !important}header .nav .menu-btn{display:inline-flex !important;align-items:center;justify-content:center}header .nav.show-menu nav{display:block !important;position:absolute;top:56px;left:var(--pad);right:var(--pad);background:#fff;border:1px solid var(--border);border-radius:12px;box-shadow:var(--shadow);padding:8px}header .nav.show-menu nav a{display:block;margin:4px 0}}",document.head.appendChild(e)}}catch{}
const t=`
<header>
  <div class="nav">
